...
public class createPostScripts extends BaseClass{
...
//		Assert.assertEquals(expected_Text0, actual_Text0);
	...
	/*
	 * @Test public void public_Distribtor() throws IOException,
	 * InterruptedException { LoginPage l=new LoginPage(driver);
	 * l.email_TF(p.getPropertyFiledata("emailid"));
	 * l.password_TF(p.getPropertyFiledata("password")); l.LoginButton();
	 * 
	 * homePage h=new homePage(driver); h.CreatePost();
	 * 
	 * createNews cn = new createNews(driver);
	 * cn.UploadImage(p.getPropertyFiledata("UploadImage6")); Point tf =
	 * cn.getTextcontentfield().getLocation(); int x = tf.getX(); int y=tf.getY();
	 * driverutilies.scrollbar(driver, x, y);
	 * cn.textContentField("public_Distribtor"); Point pblc =
	 * cn.getPublicBtn().getLocation(); int x1 = pblc.getX(); int y1 =pblc.getY();
	 * driverutilies.scrollbar(driver, x1, y1); cn.PublicButton();
	 * Thread.sleep(500); Point status = cn.getStatus().getLocation(); int x2
	 * =status.getX(); int y2 =status.getY(); driverutilies.scrollbar(driver, x2,
	 * y2); cn.Status();
	 * 
	 * cn.Distribtor(); cn.Status(); cn.PostButton(); String expected_Text1 =
	 * "News Feed"; String actual_Text1 =
